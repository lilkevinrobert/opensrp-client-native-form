...
public class MaskEditTextFactory implements FormWidgetFactory {
    ...
    public static ValidationStatus validate(JsonFormFragmentView formFragmentView,
        ...
        if (editText.isEnabled()) {
            ...
            String test = editText.getRawText();
            if (!validate || (StringUtils.isBlank(editText.getRawText()) && getRequiredValidator(editText) != null)) {
                String errorString = null;
                if (editText != null && editText.getError() != null) {
                    errorString = editText.getError().toString();
                } else if (StringUtils.isBlank(editText.getRawText())) {
                    // Patching MaskedEditText to handle case
    ...
    private static METValidator getRequiredValidator(MaskedEditText editText) {
    ...
    protected List<View> attachJson(String stepName, Context context, JsonFormFragment formFragment, JSONObject jsonObject,
        ...
        MaskedEditText editText = editTextLayout.findViewById(R.id.edit_text);
    ...
    protected int getLayout() {
        return R.layout.native_form_item_mask_edit_text;
    }

    protected void attachLayout(String stepName, Context context, JsonFormFragment formFragment,
                                final JSONObject jsonObject, final MaskedEditText editText, ImageView editButton)
            throws Exception {
        ...
        if (jsonObject.has(JsonFormConstants.MASK)) {
            editText.setMask(jsonObject.getString(JsonFormConstants.MASK));
        }
        if (jsonObject.has(JsonFormConstants.ALLOWED_CHARS)) {
            editText.setAllowedChars(jsonObject.getString(JsonFormConstants.ALLOWED_CHARS));
        }

        if (jsonObject.has(JsonFormConstants.MASK_HINT)) {
            editText.setHint(jsonObject.getString(JsonFormConstants.MASK_HINT));
        }
    ...
    private void addRequiredValidator(JSONObject jsonObject, MaskedEditText editText) throws JSONException {
    ...
    private void addRegexValidator(JSONObject jsonObject, MaskedEditText editText) throws JSONException {
    ...
    private void addEmailValidator(JSONObject jsonObject, MaskedEditText editText) throws JSONException {
        ...
        if (emailObject != null) {
            ...
            if (!TextUtils.isEmpty(emailValue) && Boolean.TRUE.toString().equalsIgnoreCase(emailValue)) {
                editText.addValidator(
                        new RegexpValidator(emailObject.getString(JsonFormConstants.ERR), Patterns.EMAIL_ADDRESS
                                .toString()));
            }
    ...
    private void addUrlValidator(JSONObject jsonObject, MaskedEditText editText) throws JSONException {
    ...
    private void addNumericValidator(JSONObject jsonObject, MaskedEditText editText) throws JSONException {
    ...
    private void addNumericIntegerValidator(JSONObject jsonObject, MaskedEditText editText) throws JSONException {
    ...
    private void addRelativeNumericIntegerValidator(JSONObject editTextJSONObject, JsonFormFragment formFragment,
                                                    MaskedEditText editText, boolean isMaxValidator) throws JSONException {
    ...
    private void addCumulativeTotalValidator(JSONObject editTextJSONObject, JsonFormFragment formFragment,
                                             final MaskedEditText editText, String stepName, JsonApi jsonApi) throws JSONException {
            ...
            if (totalFieldJSONObject != null) {
                ...
                if (relatedFieldsJson != null) {
                    ...
                    for (int i = 0; i < relatedFieldsJson.length(); i++) {
                        MaskedEditText relatedEditText = getViewUsingAddress(stepName, relatedFieldsJson.getString(i), jsonApi);
                    ...
                    MaskedEditText totalValueView = getViewUsingAddress(stepName, totalValueKey, jsonApi);
    ...
    private void attachRefreshLogic(Context context, JSONObject jsonObject, MaskedEditText editText) {
    ...
    private MaskedEditText getViewUsingAddress(String stepName, String fieldKey, JsonApi jsonApi) {
        View view = jsonApi.getFormDataView(stepName + ":" + fieldKey);
        if (view instanceof MaskedEditText)
            return (MaskedEditText) view;
        else
            return null;
    }
